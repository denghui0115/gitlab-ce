...
redis_not_listening = OmnibusHelper.new(node).not_listening?("redis")
postgresql_not_listening = OmnibusHelper.new(node).not_listening?("postgresql")
...
gitlab_workhorse_services += ['service[gitlab-workhorse]'] if OmnibusHelper.new(node).should_notify?('gitlab-workhorse')
...
file File.join(gitlab_rails_dir, "RUBY_VERSION") do
  ...
  notifies :restart, "service[unicorn]" if OmnibusHelper.new(node).should_notify?('unicorn')
end
